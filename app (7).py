...
def fetch_data(ticker, period="2y"):
    ...
    # Use adjusted close if available, otherwise fall back to regular close
    adjclose_list = result["indicators"].get("adjclose")
    if adjclose_list and len(adjclose_list) > 0:
        close_values = adjclose_list[0].get("adjclose", quotes["close"])
    else:
