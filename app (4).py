...
# ── Auto-setup: runs once when you first execute this file ──
def _setup():
...
# When run directly (python app.py), do setup + launch
if __name__ == "__main__" and "--_launched" not in sys.argv:
    _setup()
    _launch()
    sys.exit()
...
def fetch_data(ticker, period="2y"):
    ...
    if not result:
        ...
        raise ValueError(f"No data for '{ticker}': {err.get('description', 'unknown error')}")

    result    = result[0]
    quotes    = result["indicators"]["quote"][0]
    adjclose  = result["indicators"].get("adjclose", [{}])[0].get("adjclose", quotes["close"])
